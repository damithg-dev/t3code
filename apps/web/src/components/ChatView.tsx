...
import { refreshVcsStatusGroup, useVcsStatusGroups } from "~/lib/vcsStatusState";
...
function ChatViewContent(props: ChatViewProps) {
  ...
  // The refresh above only covers the anchor `gitStatusCwd`. A multi-repo
  // workspace has to fan the same post-mutation refresh out over its other repo
  // roots, or their status sections keep showing pre-turn state.
  const gitStatusRootsKey = gitStatusRoots?.join(" ") ?? "";
  const refreshVcsStatusRoots = useCallback(() => {
    refreshVcsStatusGroup({ environmentId, repoRoots: gitStatusRoots });
  }, [environmentId, gitStatusRoots]);
  useWorkspaceMutationRefresh({
    enabled: isMultiRepo,
    mutationId: workspaceMutationId,
    refresh: refreshVcsStatusRoots,
    resourceKey: `git-status-group:${activeThreadKey ?? ""}:${gitStatusRootsKey}`,
  });
