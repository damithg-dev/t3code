...
const makeNativeOperations = Effect.fn("PreviewManager.makeOperations")(function* (
  ...
  const attachListeners = Effect.fn("PreviewManager.attachListeners")(function* (
    ...
    const forwardShortcut = Effect.fn("PreviewManager.forwardShortcut")(function* (
      ...
      // The shortcut belongs to the window this guest is embedded in. Sending it
      // anywhere else would run the user's chord against a different project, so
      // an unknown or closed host means the press is dropped.
      const hostWindow = yield* findHostWindow(wc);
      if (Option.isNone(hostWindow)) return;
      event.preventDefault();
      hostWindow.value.webContents.sendInputEvent({
        type: "keyDown",
        keyCode: input.key,
        modifiers: [
