  ...
  switch (input.resource._tag) {
    ...
    case "workspace-file": {
      ...
      for (const candidate of candidateRoots) {
        ...
        matched = { workspaceRoot, relativePath: resolved.value.relativePath, canonicalFile };
        break;
      }
      if (!matched) {
      ...
      if (HEADER_IMAGE_EXTENSIONS.has(path.extname(matched.relativePath).toLowerCase())) {
        imageDimensions = yield* readImageDimensionsFromHeader(matched.canonicalFile);
      }
