...
lifecycleLayer("CodexAdapterLive lifecycle", (it) => {
  ...
  it.effect("takes the latest reset among exhausted windows and un-nests the snapshot", () =>
    Effect.gen(function* () {
      const { adapter, runtime } = yield* startLifecycleRuntime();
      const firstEventFiber = yield* Stream.runHead(adapter.streamEvents).pipe(Effect.forkChild);
      ...
      const snapshot = firstEvent.value.payload.rateLimits as Record<string, unknown>;
      NodeAssert.equal(snapshot.rateLimitReachedType, "rate_limit_reached");
      NodeAssert.equal("rateLimits" in snapshot, false);
    }),
  );
