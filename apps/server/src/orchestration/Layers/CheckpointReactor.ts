...
const make = Effect.gen(function* () {
  ...
  const resolveCheckpointRoots = Effect.fn("resolveCheckpointRoots")(function* (input: {
    ...
    const configRoots = project
      ...
      : [];
    const gitConfigRoots = yield* Effect.filter(configRoots, (root) =>
      checkpointStore.isGitRepository(root),
    );
    ...
    if (sessionCwd && (yield* checkpointStore.isGitRepository(sessionCwd))) {
      return [sessionCwd];
    }
    return [];
  });
