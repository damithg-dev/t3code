...
Each window is a full client of the same T3 Code server. Open a different project in each one and
tile them side by side, and you can watch two agents work at once instead of switching back and
forth. Because they share one server, everything stays in sync: a thread you open in one window is
the same thread in the other, and work continues in both while you look at either.

Windows are independent in the ways that matter while you work. Each has its own project, its own
thread, its own terminal focus and its own preview, and keyboard shortcuts apply to the window you
...
server for the same effect. T3 Code Mobile shows one project at a time.
